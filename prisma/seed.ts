...
async function main() {
  try {
    ...
    mouses.forEach(async (mouse) => {
      await prisma.product.create({
        data: {
          name: mouse.name,
          slug: mouse.slug,
          description: mouse.description,
          imageUrls: mouse.imageUrls,
          basePrice: mouse.basePrice,
          categoryId: mousesCategory.id,
          discountPercentage: mouse.discountPercentage,
        },
      });
    });
    ...
    headphones.forEach(async (headphone) => {
      await prisma.product.create({
        data: {
          name: headphone.name,
          slug: headphone.slug,
          description: headphone.description,
          imageUrls: headphone.imageUrls,
          basePrice: 400,
          categoryId: headphonesCategory.id,
    ...
    mousepads.forEach(async (mousePad) => {
      await prisma.product.create({
        data: {
          name: mousePad.name,
          slug: mousePad.slug,
          description: mousePad.description,
          imageUrls: mousePad.imageUrls,
          basePrice: 400,
          categoryId: mousepadsCategory.id,
          discountPercentage: mousePad.discountPercentage,
        },
      });
    });
    ...
    speakers.forEach(async (speaker) => {
      await prisma.product.create({
        data: {
          ...
          imageUrls: speaker.imageUrls,
          basePrice: 400,
          categoryId: speakersCategory.id,
          discountPercentage: speaker.discountPercentage,
        },
      });
    });
